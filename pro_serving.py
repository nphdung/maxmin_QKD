...
while len(demands_copy) > 0:
    ...
    for d in demands_copy:
        d_slot = float(d[2]/d[3])
        if d_slot < min_slot:
            min_slot = d_slot
            d_list = list()
            d_list.append(d)
        elif d_slot == min_slot:
            d_list.append(d)
    ...
    for d in d_list:
        ...
        if not nx.has_path(copy_UG,src,dst):    # check if there is a path between the source and the destination after removing some nodes
            ...
            continue   # if the network cannot satisfy the demand any more due to the insufficient capacity of edges, we consider the next demand
...
print("The maximin value:",maxmin)
print("The total keys:",total_key)
